...
### Weighted daily queue and mailbox pacing

Prepared drafts use exact Interaction Type values `Initial Message`, `Follow-up 1`, `Follow-up 2` and `Follow-up 3`. Follow-up preparation writes the numbered value after its existing due-date and reply checks; `Created At` is the queue-ready timestamp. The generic historical `Follow-up` value is unsupported and excluded with guidance. No historical records are rewritten automatically.

At the first run for a selected schedule's local date, the runner estimates the remaining global send capacity from the open window and selected interval. It allocates that capacity proportionally to eligible demand, then uses smooth weighted round-robin to interleave categories. Within a category, the oldest `Created At` wins with record ID as the stable tie-break. When an allocated category empties, spare capacity goes to the oldest waiting numbered follow-up before initials. Refreshing later runs updates remaining demand without erasing confirmed progress or replaying missed sends.

The configured schedule interval remains the global pace regardless of mailbox count. Each mailbox also has a 720-second minimum cooldown across initials and follow-ups. Initials rotate through ready connected mailboxes; follow-ups remain bound to the original sender. When every viable owner is cooling down, the run waits interruptibly until the earliest cooldown or window close. An unavailable owner never reroutes a conversation and does not block other sendable categories.

Daily fairness, confirmed category counts and per-mailbox confirmation times are stored atomically in `<SEND_ATTEMPT_FILE>.progress` (or beside the default attempt journal). Gmail confirmation is journaled before Airtable completion and progress accounting; reconciliation applies accounting idempotently before clearing the attempt. This prevents double-counting after a crash but does not claim exactly-once Gmail delivery. Keep the progress file on the same persistent private storage as the send-attempt journal.

At Start, `runStartedAt` is captured once. The repository pages a projection of Draft Interactions with Direction Outbound, Channel Email, nonblank Subject and Message, a Prospect relationship and `Created At <= runStartedAt`. The application validates complete Prospect and conversation context before including each candidate in demand. New drafts wait for the next run. Invalid relationships, unsupported types, replies, Do Not Contact and malformed ownership remain excluded with actionable run feedback.
