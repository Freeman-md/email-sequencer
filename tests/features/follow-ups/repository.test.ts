...
import { InteractionsRepository } from '@/features/follow-ups/server/repositories/interactions.repository';
...
it('creates a linked Draft with the original subject/thread and no send identifiers or timestamps', async () => {
  ...
  const repository = new InteractionsRepository({ request });
...
it('loads linked history in bounded ID batches and rejects incomplete history', async () => {
  ...
  const repository = new InteractionsRepository({ request });
  expect(await repository.history(ids)).toHaveLength(51);
  ...
  await expect(repository.history(['recMissing'])).rejects.toThrow(
    'incomplete',
  );
});
